...
export const GET = isCreator((event) => {
	...
	).then((chapters) => new Response(JSON.stringify(chapters), { status: 200 }));
});
...
export const POST = isCreator(async (event) => {
	...
	return run((client) => createChapter(client, courseId, data)).then(
		(chapter) => new Response(JSON.stringify(chapter), { status: 201 })
	);
});
